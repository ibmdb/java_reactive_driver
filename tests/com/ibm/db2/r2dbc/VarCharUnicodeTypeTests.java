...
public class VarCharUnicodeTypeTests extends BaseTestCase
{
	private static final Logger _logger =  LoggerFactory.getLogger(VarCharUnicodeTypeTests.class.getName());

	public VarCharUnicodeTypeTests()
	{
		super();
	...
	public static void createUNICODETableA()
	{
		...
		Mono<Void> mono = getNewConnection()
								...
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_U_A_"+_randomInt + " (NAME VARCHAR(100)) CCSID UNICODE"));
	...
	public static void createUNICODETableB()
	{
		...
		String createTable;
		if (_isLUW) {
			// FOR SBCS DATA is invalid for LUW
			createTable = "CREATE TABLE UNIT_TEST_U_B_"+_randomInt + " (NAME VARCHAR(100)) CCSID UNICODE";
		}
		else {
			createTable = "CREATE TABLE UNIT_TEST_U_B_"+_randomInt + " (NAME VARCHAR(100) FOR SBCS DATA) CCSID UNICODE";
		}
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
	...
	public static void createUNICODETableC()
	{
		...
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_U_C_"+_randomInt + " (NAME VARCHAR(100) FOR MIXED DATA) CCSID UNICODE"));
	...
	public static void createUNICODETableD()
	{
		...
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_U_D_"+_randomInt + " (NAME VARCHAR(100) FOR BIT DATA) CCSID UNICODE"));
	...
	/**
	 * Test querying for the data inserted. In LUW, BIT DATA has limitations, hence this test is skipped.
	 */
	@Test
