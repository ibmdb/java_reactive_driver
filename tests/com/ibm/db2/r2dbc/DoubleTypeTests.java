...
public class DoubleTypeTests extends BaseTestCase
{
	private static final Logger _logger =  LoggerFactory.getLogger(DoubleTypeTests.class.getName());
	
	private static Double MAX_VALUE = 7.2E+75;  // max value that DB2 can handle
	private static Double MIN_VALUE = 5.4E-79;  // smallest value DB2 can handle
	...
	public static void createTable()
	{
		...
		Mono<Void> mono = getNewConnection()
								...
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_"+_randomInt + " (ID INTEGER, AMOUNT DOUBLE NOT NULL, ALIAS_AMOUNT DOUBLE)"));
	...
	@Test  
	public void test001CreateTableWithDoubleColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_1_"+_randomInt + " (ID DOUBLE)"));
	...
	@Test  
	public void test002DropTableWithDoubleColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
	...
	public static class Employee 
	{
		...
		public Employee(int id, double deptId)
		{
			_id = id;
			_amount = deptId;
		}
	...
	public void test201Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	...
					 	assert row.get("AMOUNT", Double.class).compareTo(s) == 0: "Expected value is 12.5, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test203Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	...
					 	assert row.get("AMOUNT", Double.class).compareTo(s) == 0: "Expected value is 12.99, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test205Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									...
									.flatMap(c -> c.createStatement("SELECT ID, AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ID = 2010")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("AMOUNT", Double.class))))
								   .doAfterTerminate(() -> con.get().release());
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
						...
						assert emp.getAmount() - s < 0.0001: "Expected time value is 123.4, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test208Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
					 	...
					 	assert emp instanceof Employee : "Did not receive an Employee object";
						assert emp.getId() == 2010 : "Expected value 100, but received "+emp.getId();
						assert emp.getAmount().compareTo(s) == 0: "Expected time value is 7.2E+75, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test209Select() 
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("SELECT ID, AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE AMOUNT = 7.2E+75")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("AMOUNT", Double.class))))
								   .doAfterTerminate(() -> con.get().release());
	...
	/**
	 * Test querying for the data inserted using a smallest bind value and see if we could query it. (boundary condition)
	 */
	@Test
	public void test210Select() 
	{
		...
		// Get connection insert data and release the connection used 
		Double s = MIN_VALUE;  // actually smallest representable value
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  .bind(1, 2010)
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
					 	...
					 	assert emp instanceof Employee : "Did not receive an Employee object";
						assert emp.getId() == 2010 : "Expected value 100, but received "+emp.getId();
						assert emp.getAmount().compareTo(s) == 0: "Expected time value is 5.4E-79, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	/**
	 * Test querying for the data inserted using a smallest bind value and see if we could query using this value in where clause. (boundary condition)
	 */
	@Test
	public void test212Select()
	{
		_logger.debug("running test212Select");
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
					 	...
					 	assert emp instanceof Employee : "Did not receive an Employee object";
						assert emp.getId() == 2010 : "Expected value 100, but received "+emp.getId();
						assert emp.getAmount().compareTo(s) == 0: "Expected time value is 1.4E-45, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test301Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("AMOUNT", Double.class) - s2 < 0.0001: "Expected value is 25.05, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test302Update()
	{
		...
		// Test
		Double s2 = new Double("25.75");
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("AMOUNT", Double.class).equals(s2) : "Expected value is 25.75, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	{	
		...
		// Test
		Double s2 = new Double("-75.25");
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("AMOUNT", Double.class).compareTo(s2) == 0: "Expected value is -75.25, but received = "+row.get("ID");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test304Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("ALIAS_AMOUNT", Double.class).compareTo(s2) == 0: "Expected value is 15, but received = "+row.get("ALIAS_AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test305Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("ALIAS_AMOUNT", Double.class) == null : "Expected value is NULL, but received = "+row.get("ALIAS_AMOUNT");
					})
					.expectComplete()
					.verify();
	}
