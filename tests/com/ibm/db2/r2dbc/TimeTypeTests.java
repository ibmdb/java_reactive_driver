...
public class TimeTypeTests extends BaseTestCase
{
	private static final Logger _logger =  LoggerFactory.getLogger(TimeTypeTests.class.getName());

	public TimeTypeTests()
	{
		super();
	...
	public static void createTable()
	{
		...
		Mono<Void> mono = getNewConnection()
								...
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_"+_randomInt + " (ID INTEGER, START_TIME TIME NOT NULL, END_TIME TIME)"));
	...
	@Test  
	public void test001CreateTableWithTimeColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_1_"+_randomInt + " (T TIME)"));
	...
	@Test  
	public void test002DropTableWithTimeColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
	...
	{	
		...
		Mono<Void> mono = _pool.getConnection()
							   .doOnNext(c -> con.set(c))
							   .flatMap(c -> c.executeUpdate("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, START_TIME) VALUES (100, '07:30:15')"))
	...
	public void test102Insert()
	{
		...
		Mono<Void> mono = getNewConnection()
								...
								.flatMap(c -> c.executeUpdate("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, START_TIME) VALUES (200, '07:30:15')"));
	...
	public void test103Insert()
	{
		...
		Mono<DB2Result> mono = getNewConnection()
								   .doOnNext(c -> {con.set(c); _con = c;})
								   .flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, START_TIME) VALUES (25, '07:30:15')")
												  .execute());
	...
	public void test104Insert()
	{
		...
		Time time = Time.valueOf("07:30:15");
	...
	{	
		...
		Mono<Void> mono = _pool.getConnection()
							   .doOnNext(c -> con.set(c))
							   .flatMap(c -> c.executeUpdate("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, START_TIME, END_TIME) VALUES (100, '07:30:15', NULL)"))
							   .doAfterTerminate(() -> con.get().release()); 
	...
	public static class Employee 
	{
		...
		public Employee(int id, Time startTime)
		{
			_id = id;
	...
	public void test201Select()
	{
		...
		Time time = Time.valueOf("12:45:30");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, START_TIME) VALUES (?, ?)")
										   		  .bind(1, 2001)
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	...
					 	assert row.get("START_TIME", Time.class).equals(time) : "Expected value is 12:45:30, but received = "+row.get("START_TIME");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test204Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	...
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert (Integer)row.get("ID") == 2003 : "Expected value is 2003, but received = "+row.get("ID");
					 	assert row.get("START_TIME", Time.class).equals(time) : "Expected value is 2019-10-02, but received = "+row.get("START_TIME");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test210Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									...
									.flatMap(c -> c.createStatement("SELECT ID, START_TIME FROM UNIT_TEST_"+_randomInt+" WHERE ID = 2010")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("START_TIME", Time.class))))
								   .doAfterTerminate(() -> con.get().release());
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
						...
						assert emp.getStartTime().equals(time) : "Expected time value is 12:45:30, but received "+emp.getStartTime();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test301Update()
	{
		...
		// Get connection and insert data
		Time time1 = Time.valueOf("12:45:30");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, START_TIME) VALUES (?, ?)")
										   		  .bind(1, 3001)
		...
		Time time2 = Time.valueOf("11:25:45");
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("START_TIME", Time.class).equals(time2) : "Expected value is '11:25:45, but received = "+row.get("START_TIME");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test302Update()
	{
		...
		Mono<DB2Result> mono2 = Mono.just(con.get())
								   	.flatMap(c -> c.createStatement("UPDATE UNIT_TEST_"+_randomInt+" SET START_TIME = '11:25:45' WHERE ID = 3002")
												  	.execute());
	...
	{	
		...
		Mono<Void> mono = _pool.getConnection()
							   .doOnNext(c -> con.set(c))
							   .flatMap(c -> c.executeUpdate("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, START_TIME) VALUES (3003, '12:45:30')"));
		...
		Mono<Void> mono2 = Mono.just(con.get())
							   .flatMap(c -> c.executeUpdate("UPDATE UNIT_TEST_"+_randomInt + " SET START_TIME = '11:25:45' WHERE ID = 3003"));
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("START_TIME", Time.class).equals(time2) : "Expected value is 3004, but received = "+row.get("ID");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test304Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("END_TIME", Time.class).equals(time2) : "Expected value is '2019-12-30, but received = "+row.get("END_TIME");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test305Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("END_TIME", Time.class) == null : "Expected value is '2019-12-30, but received = "+row.get("END_TIME");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test401Delete()
	{
		...
		// Get connection and insert data
		Time time = Time.valueOf("11:25:45");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, START_TIME) VALUES (?, ?)")
										   		  .bind(1, 5001)
	...
	public void test402Delete()
	{
		...
		// Delete the inserted data
		Mono<DB2Result> mono2 = Mono.just(con.get())
								   	.flatMap(c -> c.createStatement("DELETE FROM UNIT_TEST_"+_randomInt+" WHERE START_TIME = '11:25:45'")
												  	.execute());
		...
		// Query the deleted data, we should get no rows and then release the connection
		Flux<Row> flux = Mono.just(con.get())
							   .flatMap(c -> c.createStatement("SELECT ID FROM UNIT_TEST_"+_randomInt+" WHERE START_TIME = '11:25:45'")
											  .execute())
							   .flatMapMany(result -> result.map((row, md) -> row))
							   .doAfterTerminate(() -> con.get().release());
	...
	{	
		...
		Mono<Void> mono = _pool.getConnection()
							   .doOnNext(c -> con.set(c))
							   .flatMap(c -> c.executeUpdate("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, START_TIME) VALUES (5003, '11:25:45')"));
		...
		// delete data 
		Mono<Void> mono2 = Mono.just(con.get())
							   .flatMap(c -> c.executeUpdate("DELETE FROM UNIT_TEST_"+_randomInt + " WHERE START_TIME = '11:25:45'"));
