...
public class CharAsciiTypeTests extends BaseTestCase
{
	private static final Logger _logger =  LoggerFactory.getLogger(CharAsciiTypeTests.class.getName());

	public CharAsciiTypeTests()
	{
		super();
	...
	public static void createASCIITableA()
	{
		...
		String createTable;
		if (_isLUW) {
			// CCSID ASCII is not valid for LUW
			createTable = "CREATE TABLE UNIT_TEST_A_A_"+_randomInt + " (NAME CHAR(25))";
		} else {
			createTable = "CREATE TABLE UNIT_TEST_A_A_"+_randomInt + " (NAME CHAR(25)) CCSID ASCII";
		}
	...
	public static void createASCIITableB()
	{
		...
		String createTable;
		if (_isLUW) {
			// FOR SBCS and CCSID ASCII specification are not allowed in LUW
			createTable = "CREATE TABLE UNIT_TEST_A_B_"+_randomInt + " (NAME CHAR(25))";
		}
		else {
			createTable = "CREATE TABLE UNIT_TEST_A_B_"+_randomInt + " (NAME CHAR(25) FOR SBCS DATA) CCSID ASCII";
		}
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
								.flatMap(c -> c.executeUpdate(createTable));
	...
	public static void createASCIITableC()
	{
		...
		String createTable;
		if (_isLUW) {
			// CCSID ASCII specification is not allowed in LUW also BIT DATA has limitations
			createTable = "CREATE TABLE UNIT_TEST_A_C_"+_randomInt + " (NAME CHAR(25))";
		}
		else {
			createTable = "CREATE TABLE UNIT_TEST_A_C_"+_randomInt + " (NAME CHAR(25) FOR BIT DATA) CCSID ASCII";
		}
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
								.flatMap(c -> c.executeUpdate(createTable));
