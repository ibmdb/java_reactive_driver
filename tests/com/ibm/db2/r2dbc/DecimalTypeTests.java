...
public class DecimalTypeTests extends BaseTestCase
{
	private static final Logger _logger =  LoggerFactory.getLogger(DecimalTypeTests.class.getName());
	...
	public static void createTable()
	{
		...
		Mono<Void> mono = getNewConnection()
								...
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_"+_randomInt + " (ID INTEGER, AMOUNT DECIMAL(5,2) NOT NULL, ALIAS_AMOUNT DECIMAL(5,2))"));
		
		StepVerifier.create(mono)
		            .expectComplete()
		            .verify();	
		
		mono = getNewConnection()
				.doOnNext(c -> con.set(c))
				.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_2_"+_randomInt + " (ID INTEGER, AMOUNT DECIMAL(10,2) NOT NULL, ALIAS_AMOUNT DECIMAL(10,2))"));
		...
		mono = getNewConnection()
				.doOnNext(c -> con.set(c))
				.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_3_"+_randomInt + " (ID INTEGER, AMOUNT DECIMAL(9,2) NOT NULL, ALIAS_AMOUNT DECIMAL(10,2))"));
	...
	@Test  
	public void test001CreateTableWithDecimalColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_1_"+_randomInt + " (ID DECIMAL(5,2))"));
	...
	@Test  
	public void test002DropTableWithDecimalColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
	...
	{	
		...
		Mono<Void> mono = _pool.getConnection()
							   .doOnNext(c -> con.set(c))
							   .flatMap(c -> c.executeUpdate("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (100, 100.25)"))
	...
	public void test102Insert()
	{
		...
		Mono<Void> mono = getNewConnection()
								...
								.flatMap(c -> c.executeUpdate("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (200, 90.25)"));
	...
	public void test103Insert()
	{
		...
		Mono<DB2Result> mono = getNewConnection()
								   .doOnNext(c -> {con.set(c); _con = c;})
								   .flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (25, 9.98)")
												  .execute());
	...
	public void test104Insert()
	{
		...
		BigDecimal s = new BigDecimal("1.25");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  	.bind(1, 35)
										   		  	.bind(2, s)
										   		  	.execute())
									.doAfterTerminate(() -> con.get().release());
	 ...
	 */
	@Test
	public void test1041Insert()
	{
		_logger.debug("running test1041Insert");
		
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		// get connection, insert data and then release the connection
		BigDecimal s = new BigDecimal("1.250");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  	.bind(1, 35)
	 ...
	 */
	@Test
	public void test1042Insert()
	{
		_logger.debug("running test1042Insert");
		
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		// get connection, insert data and then release the connection
		BigDecimal s = new BigDecimal("1.251");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  	.bind(1, 35)
	 ...
	 */
	@Test
	public void test1043Insert()
	{
		_logger.debug("running test1043Insert");
		
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		// get connection, insert data and then release the connection
		BigDecimal s = new BigDecimal("1234.25");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  	.bind(1, 35)
	 ...
	 */
	@Test
	public void test1044Insert()
	{
		_logger.debug("running test1044Insert");
		
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		// get connection, insert data and then release the connection
		BigDecimal s = new BigDecimal("1234.2");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  	.bind(1, 35)
	 ...
	 */
	@Test
	public void test1045Insert()
	{
		_logger.debug("running test1045Insert");
		
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		// get connection, insert data and then release the connection
		BigDecimal s = new BigDecimal("200.00");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_2_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  	.bind(1, 35)
										   		  	.bind(2, s)
										   		  	.execute())
									.doAfterTerminate(() -> con.get().release());
	 ...
	 */
	@Test
	public void test1046Insert()
	{
		_logger.debug("running test1046Insert");
		
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		// get connection, insert data and then release the connection
		BigDecimal s = new BigDecimal("20.00");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_2_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  	.bind(1, 35)
	 ...
	 */
	@Test
	public void test1047Insert()
	{
		_logger.debug("running test1047Insert");
		...
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_3_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  	.bind(1, 35)
										   		  	.bind(2, s)
										   		  	.execute())
									.doAfterTerminate(() -> con.get().release());
	 ...
	 */
	@Test
	public void test1048Insert()
	{
		_logger.debug("running test1048Insert");
	...
	{	
		...
		Mono<Void> mono = _pool.getConnection()
							   .doOnNext(c -> con.set(c))
							   .flatMap(c -> c.executeUpdate("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT, ALIAS_AMOUNT) VALUES (100, -10.12, NULL)"))
							   .doAfterTerminate(() -> con.get().release()); 
	...
	public void test106Insert()
	{
		...
		// get connection, insert data and then release the connection
		BigDecimal s = new BigDecimal("12.5");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT, ALIAS_AMOUNT) VALUES (?, ?, ?)")
										   		  	.bind(1, 35)
										   		  	.bind(2, s)
										   		  	.bindNull(3, BigDecimal.class)
										   		  	.execute())
									.doAfterTerminate(() -> con.get().release());
	...
	public void test110Insert()
	{
		...
		// get connection, try inserting data and then release the connection
		BigDecimal s = new BigDecimal("999.99");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  	.bind(1, 35)
	...
	public static class Employee 
	{
		...
		public Employee(int id, BigDecimal deptId)
		{
			_id = id;
			_amount = deptId;
		}
	...
	public void test201Select()
	{
		...
		Flux<Row> flux = Mono.just(con.get())
							   .flatMap(c -> c.createStatement("SELECT ID, AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ID = 2001")
											  .execute())
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	...
					 	assert row.get("AMOUNT", BigDecimal.class).compareTo(s) == 0: "Expected value is 12.5, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test202Select()
	{
		...
		Flux<Row> flux = getNewConnection()
							    ...
							    .flatMap(c -> c.createStatement("SELECT ID, AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ID = 2002")
								 			  	.execute())
							    .flatMapMany(result -> result.map((row, md) -> row));
	...
	public void test203Select()
	{
		...
		// Get connection, insert data and then release the connection  
		BigDecimal s = new BigDecimal("12.99");  
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
											   		  .bind(1, 2003)
		...
		Flux<Row> flux = _pool.getConnection()
							  .doOnNext(c -> con.set(c))
							  .flatMap(c -> c.createStatement("SELECT ID, AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ID = 2003")
									  			.execute())
							  .flatMapMany(result -> result.map((row, md) -> row))
							  .doAfterTerminate(() -> con.get().release());
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	...
					 	assert row.get("AMOUNT", BigDecimal.class).compareTo(s) == 0: "Expected value is 12.99, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test204Select()
	{
		...
		Flux<Row> flux = _pool.getConnection()
							  .doOnNext(c -> con.set(c))
							  .flatMap(c -> c.createStatement("SELECT ID, AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ALIAS_AMOUNT is NULL")
									  			.execute())
							  .flatMapMany(result -> result.map((row, md) -> row))
							  .doAfterTerminate(() -> con.get().release());
	...
	public void test205Select()
	{
		...
		BigDecimal s = new BigDecimal("123.4");  
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  .bind(1, 2010)
		...
		Flux<Employee> flux = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("SELECT ID, AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ID = 2010")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("AMOUNT", BigDecimal.class))))
								   .doAfterTerminate(() -> con.get().release());
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
						...
						assert emp.getAmount().compareTo(s) == 0: "Expected time value is 123.4, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test206Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("SELECT ID, AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE AMOUNT = ?")
											        .bind(1, s)
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("AMOUNT", BigDecimal.class))))
								   .doAfterTerminate(() -> con.get().release());
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
					 	...
					 	assert emp instanceof Employee : "Did not receive an Employee object";
						assert emp.getId() == 2010 : "Expected value 100, but received "+emp.getId();
						assert emp.getAmount().compareTo(s) == 0: "Expected time value is 12.5, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test208Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
					 	...
					 	assert emp instanceof Employee : "Did not receive an Employee object";
						assert emp.getId() == 2010 : "Expected value 100, but received "+emp.getId();
						assert emp.getAmount().compareTo(s) == 0: "Expected time value is 999.99, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test209Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("SELECT ID, AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE AMOUNT = +999.99")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("AMOUNT", BigDecimal.class))))
								   .doAfterTerminate(() -> con.get().release());
	...
	public void test210Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
					 	...
					 	assert emp instanceof Employee : "Did not receive an Employee object";
						assert emp.getId() == 2010 : "Expected value 100, but received "+emp.getId();
						assert emp.getAmount().compareTo(s) == 0: "Expected time value is 10, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test301Update()
	{
		...
		// Get connection and insert data
		BigDecimal s1 = new BigDecimal("12.5");
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (?, ?)")
										   		  .bind(1, 3001)
		...
		BigDecimal s2 = new BigDecimal("25.05");
		Mono<DB2Result> mono2 = Mono.just(con.get())
								   	.flatMap(c -> c.createStatement("UPDATE UNIT_TEST_"+_randomInt+" SET AMOUNT = ? WHERE ID = 3001")
								   					.bind(1, s2)
												  	.execute());
		...
		Flux<Row> flux = Mono.just(con.get())
							   .flatMap(c -> c.createStatement("SELECT AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ID = 3001")
											  .execute())
							   .flatMapMany(result -> result.map((row, md) -> row))
							   .doAfterTerminate(() -> con.get().release());
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("AMOUNT", BigDecimal.class).compareTo(s2) == 0: "Expected value is 15, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test302Update()
	{
		...
		Mono<DB2Result> mono2 = Mono.just(con.get())
								   	.flatMap(c -> c.createStatement("UPDATE UNIT_TEST_"+_randomInt+" SET AMOUNT = 25.75 WHERE ID = 3002")
												  	.execute());
		...
		// Query the updated data and then release the connection
		Flux<Row> flux = Mono.just(con.get())
							   .flatMap(c -> c.createStatement("SELECT AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ID = 3002")
											  .execute())
							   .flatMapMany(result -> result.map((row, md) -> row))
							   .doAfterTerminate(() -> con.get().release());

		// Test
		BigDecimal s2 = new BigDecimal("25.75");
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("AMOUNT", BigDecimal.class).equals(s2) : "Expected value is 25.75, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	{	
		...
		Mono<Void> mono = _pool.getConnection()
							   .doOnNext(c -> con.set(c))
							   .flatMap(c -> c.executeUpdate("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (3003, 12.5)"));
		...
		Mono<Void> mono2 = Mono.just(con.get())
							   .flatMap(c -> c.executeUpdate("UPDATE UNIT_TEST_"+_randomInt + " SET AMOUNT = -75.25 WHERE ID = 3003"));
		...
		Flux<Row> flux = Mono.just(con.get())
							   .flatMap(c -> c.createStatement("SELECT AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ID = 3003")
											  .execute())
							   .flatMapMany(result -> result.map((row, md) -> row))
							   .doAfterTerminate(() -> con.get().release());
		
		// Test
		BigDecimal s2 = new BigDecimal("-75.25");
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("AMOUNT", BigDecimal.class).compareTo(s2) == 0: "Expected value is -75.25, but received = "+row.get("ID");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test304Update()
	{
		...
		BigDecimal s2 = new BigDecimal("75.25");
		Mono<DB2Result> mono2 = Mono.just(con.get())
								   	.flatMap(c -> c.createStatement("UPDATE UNIT_TEST_"+_randomInt+" SET ALIAS_AMOUNT = ? WHERE ID = 3001")
								   					.bind(1, s2)
												  	.execute());
		...
		// Query the updated data and then release the connection
		Flux<Row> flux = Mono.just(con.get())
							   .flatMap(c -> c.createStatement("SELECT ALIAS_AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ID = 3001")
											  .execute())
							   .flatMapMany(result -> result.map((row, md) -> row))
							   .doAfterTerminate(() -> con.get().release());
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("ALIAS_AMOUNT", BigDecimal.class).compareTo(s2) == 0: "Expected value is 15, but received = "+row.get("ALIAS_AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test305Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("ALIAS_AMOUNT", BigDecimal.class) == null : "Expected value is NULL, but received = "+row.get("ALIAS_AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test401Delete()
	{
		...
		Mono<DB2Result> mono2 = Mono.just(con.get())
								   	.flatMap(c -> c.createStatement("DELETE FROM UNIT_TEST_"+_randomInt+" WHERE AMOUNT = ?")
								   					.bind(1, s1) 
												  	.execute());
		// Test
		StepVerifier.create(mono2)
		...
		// Query the deleted data, we should get no rows and then release the connection
		Flux<Row> flux = Mono.just(con.get())
							   .flatMap(c -> c.createStatement("SELECT ID FROM UNIT_TEST_"+_randomInt+" WHERE AMOUNT = ?")
									   		  .bind(1, s1)
											  .execute())
							   .flatMapMany(result -> result.map((row, md) -> row))
							   .doAfterTerminate(() -> con.get().release());
	...
	public void test402Delete()
	{
		...
		// Delete the inserted data
		Mono<DB2Result> mono2 = Mono.just(con.get())
								   	.flatMap(c -> c.createStatement("DELETE FROM UNIT_TEST_"+_randomInt+" WHERE AMOUNT = 12.5")
												  	.execute());
		...
		// Query the deleted data, we should get no rows and then release the connection
		Flux<Row> flux = Mono.just(con.get())
							   .flatMap(c -> c.createStatement("SELECT ID FROM UNIT_TEST_"+_randomInt+" WHERE AMOUNT = 12.5")
											  .execute())
							   .flatMapMany(result -> result.map((row, md) -> row))
							   .doAfterTerminate(() -> con.get().release());
	...
	{	
		...
		Mono<Void> mono = _pool.getConnection()
							   .doOnNext(c -> con.set(c))
							   .flatMap(c -> c.executeUpdate("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, AMOUNT) VALUES (5003, -12.5)"));
		...
		// delete data 
		Mono<Void> mono2 = Mono.just(con.get())
							   .flatMap(c -> c.executeUpdate("DELETE FROM UNIT_TEST_"+_randomInt + " WHERE AMOUNT = -12.5"));
	...
	public void test404Delete()
	{
		...
		// Query the deleted data, we should get no rows and then release the connection
		Flux<Row> flux = Mono.just(con.get())
							   .flatMap(c -> c.createStatement("SELECT ID FROM UNIT_TEST_"+_randomInt+" WHERE ALIAS_AMOUNT = ?")
									   		  .bindNull(1, BigDecimal.class)
											  .execute())
							   .flatMapMany(result -> result.map((row, md) -> row))
							   .doAfterTerminate(() -> con.get().release());
