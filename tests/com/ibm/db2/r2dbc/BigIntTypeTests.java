...
public class BigIntTypeTests extends BaseTestCase
{
	private static final Logger _logger =  LoggerFactory.getLogger(BigIntTypeTests.class.getName());

	public BigIntTypeTests()
	{
		super();
	...
	public static void createTable()
	{
		...
		Mono<Void> mono = getNewConnection()
								...
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_"+_randomInt + " (ID INTEGER, DEPT_ID BIGINT NOT NULL, ALIAS_DEPT_ID BIGINT)"));
	...
	@Test  
	public void test001CreateTableWithBigIntColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_1_"+_randomInt + " (ID BIGINT)"));
	...
	@Test  
	public void test002DropTableWithBigIntColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
	 ...
	 */
	@Test
	public void test1081Insert()
	{
		_logger.debug("running test1081Insert");
	...
	/**
	 * Test inserting a row using with a bind value of wrong type.
	 */
	@Test
	public void test1082Insert()
	{
		_logger.debug("running test1082Insert");
		...
		Mono<DB2Result> mono = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("INSERT INTO UNIT_TEST_"+_randomInt + " (ID, DEPT_ID) VALUES (?, ?)")
										   		  	.bind(1, "one")  // should be Integer
										   		  	.bind(2, s) // should be a Long
										   		  	.execute())
									.doAfterTerminate(() -> con.get().release());
	...
	public static class Employee 
	{
		...
		public Employee(int id, long deptId)
		{
			_id = id;
			_deptId = deptId;
		}
		...
		public Long getDeptId() {return _deptId;}
	...
	public void test201Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	...
					 	assert row.get("DEPT_ID", Long.class).equals(s) : "Expected value is 10, but received = "+row.get("DEPT_ID");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test205Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									...
									.flatMap(c -> c.createStatement("SELECT ID, DEPT_ID FROM UNIT_TEST_"+_randomInt+" WHERE ID = 2010")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("DEPT_ID", Long.class))))
								   .doAfterTerminate(() -> con.get().release());
	...
	public void test209Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("SELECT ID, DEPT_ID FROM UNIT_TEST_"+_randomInt+" WHERE DEPT_ID = 9223372036854775807")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("DEPT_ID", Long.class))))
								   .doAfterTerminate(() -> con.get().release());
	...
	public void test211Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("SELECT ID, DEPT_ID FROM UNIT_TEST_"+_randomInt+" WHERE DEPT_ID = -9223372036854775808")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("DEPT_ID", Long.class))))
								   .doAfterTerminate(() -> con.get().release());
	...
	public void test301Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("DEPT_ID", Long.class).equals(s2) : "Expected value is 15, but received = "+row.get("DEPT_ID");
					})
					.expectComplete()
					.verify();
	}
	...
	{	
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("DEPT_ID", Long.class).equals(s2) : "Expected value is 3004, but received = "+row.get("ID");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test304Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("ALIAS_DEPT_ID", Long.class).equals(s2) : "Expected value is 15, but received = "+row.get("ALIAS_DEPT_ID");
					})
					.expectComplete()
					.verify();
	}
