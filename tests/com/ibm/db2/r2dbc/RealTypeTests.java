...
public class RealTypeTests extends BaseTestCase
{
	private static final Logger _logger =  LoggerFactory.getLogger(RealTypeTests.class.getName());

	public RealTypeTests()
	{
		super();
	...
	public static void createTable()
	{
		...
		Mono<Void> mono = getNewConnection()
								...
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_"+_randomInt + " (ID INTEGER, AMOUNT REAL NOT NULL, ALIAS_AMOUNT REAL)"));
	...
	@Test  
	public void test001CreateTableWithRealColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_1_"+_randomInt + " (ID REAL)"));
	...
	@Test  
	public void test002DropTableWithRealColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
	...
	public static class Employee 
	{
		...
		public Employee(int id, float deptId)
		{
			_id = id;
			_amount = deptId;
		}
	...
	public void test201Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	...
					 	assert row.get("AMOUNT", Float.class).compareTo(s) == 0: "Expected value is 12.5, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test203Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	...
					 	assert row.get("AMOUNT", Float.class).compareTo(s) == 0: "Expected value is 12.99, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test205Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									...
									.flatMap(c -> c.createStatement("SELECT ID, AMOUNT FROM UNIT_TEST_"+_randomInt+" WHERE ID = 2010")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("AMOUNT", Float.class))))
								   .doAfterTerminate(() -> con.get().release());
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
						...
						assert emp.getAmount() - s < 0.0001: "Expected value is 123.4, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test206Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
					 	...
					 	assert emp instanceof Employee : "Did not receive an Employee object";
						assert emp.getId() == 2010 : "Expected value 100, but received "+emp.getId();
						assert emp.getAmount().compareTo(s) == 0: "Expected value is 12.5, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test208Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
					 	...
					 	assert emp instanceof Employee : "Did not receive an Employee object";
						assert emp.getId() == 2010 : "Expected value 100, but received "+emp.getId();
						assert emp.getAmount().compareTo(s) == 0: "Expected value is 3.4028235E38, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test210Select()
	{
		...
		// Get connection insert data and release the connection used 
		// actually smallest representable value
		Float s; 
		if (_isLUW) {
			s = new Float(1.4E-38);
		} else {
		...
		StepVerifier.create(flux)
					.assertNext(emp -> {
					 	...
					 	assert emp instanceof Employee : "Did not receive an Employee object";
						assert emp.getId() == 2010 : "Expected value 100, but received "+emp.getId();
						assert emp.getAmount().compareTo(s) == 0: "Expected value is 1.4E-45, but received "+emp.getAmount();
					})
					.expectComplete()
					.verify();
	}
	...
	public void test301Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("AMOUNT", Float.class) - s2 < 0.0001: "Expected value is 25.05, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test302Update()
	{
		...
		// Test
		Float s2 = new Float("25.75");
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("AMOUNT", Float.class).equals(s2) : "Expected value is 25.75, but received = "+row.get("AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	{	
		...
		// Test
		Float s2 = new Float("-75.25");
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("AMOUNT", Float.class).compareTo(s2) == 0: "Expected value is -75.25, but received = "+row.get("ID");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test304Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("ALIAS_AMOUNT", Float.class).compareTo(s2) == 0: "Expected value is 15, but received = "+row.get("ALIAS_AMOUNT");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test305Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("ALIAS_AMOUNT", Float.class) == null : "Expected value is NULL, but received = "+row.get("ALIAS_AMOUNT");
					})
					.expectComplete()
					.verify();
	}
