...
public class SmallIntTypeTests extends BaseTestCase
{
	private static final Logger _logger =  LoggerFactory.getLogger(SmallIntTypeTests.class.getName());

	public SmallIntTypeTests()
	{
		super();
	...
	public static void createTable()
	{
		...
		Mono<Void> mono = getNewConnection()
								...
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_"+_randomInt + " (ID INTEGER, DEPT_ID SMALLINT NOT NULL, ALIAS_DEPT_ID SMALLINT)"));
	...
	@Test  
	public void test001CreateTableWithSmallIntColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
		
		Mono<Void> mono = getNewConnection()
								.doOnNext(c -> con.set(c))
								.flatMap(c -> c.executeUpdate("CREATE TABLE UNIT_TEST_1_"+_randomInt + " (ID SMALLINT)"));
	...
	@Test  
	public void test002DropTableWithSmallIntColumn()
	{
		AtomicReference<DB2Connection> con = new AtomicReference<DB2Connection>();
	...
	public static class Employee 
	{
		...
		public Employee(int id, short deptId)
		{
			_id = id;
			_deptId = deptId;
		}
		...
		public Short getDeptId() {return _deptId;}
	...
	public void test201Select()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	...
					 	assert row.get("DEPT_ID", Short.class).equals(s) : "Expected value is 10, but received = "+row.get("DEPT_ID");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test205Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									...
									.flatMap(c -> c.createStatement("SELECT ID, DEPT_ID FROM UNIT_TEST_"+_randomInt+" WHERE ID = 2010")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("DEPT_ID", Short.class))))
								   .doAfterTerminate(() -> con.get().release());
	...
	public void test209Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("SELECT ID, DEPT_ID FROM UNIT_TEST_"+_randomInt+" WHERE DEPT_ID = 32767")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("DEPT_ID", Short.class))))
								   .doAfterTerminate(() -> con.get().release());
	...
	public void test211Select()
	{
		...
		Flux<Employee> flux = _pool.getConnection()
									.doOnNext(c -> con.set(c))
									.flatMap(c -> c.createStatement("SELECT ID, DEPT_ID FROM UNIT_TEST_"+_randomInt+" WHERE DEPT_ID = -32768")
													.execute())
								   .flatMapMany(result -> result.map((row, md) -> new Employee(row.get("ID", Integer.class), row.get("DEPT_ID", Short.class))))
								   .doAfterTerminate(() -> con.get().release());
	...
	public void test301Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("DEPT_ID", Short.class).equals(s2) : "Expected value is 15, but received = "+row.get("DEPT_ID");
					})
					.expectComplete()
					.verify();
	}
	...
	{	
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("DEPT_ID", Short.class).equals(s2) : "Expected value is 3004, but received = "+row.get("ID");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test304Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("ALIAS_DEPT_ID", Short.class).equals(s2) : "Expected value is 15, but received = "+row.get("ALIAS_DEPT_ID");
					})
					.expectComplete()
					.verify();
	}
	...
	public void test305Update()
	{
		...
		StepVerifier.create(flux)
					.assertNext(row -> {
					 	assert row != null : "Received a null object";
					 	assert row instanceof Row : "Did not receive a Row object";
					 	assert row.get("ALIAS_DEPT_ID", Short.class) == null : "Expected value is NULL, but received = "+row.get("ALIAS_DEPT_ID");
					})
					.expectComplete()
					.verify();
	}
